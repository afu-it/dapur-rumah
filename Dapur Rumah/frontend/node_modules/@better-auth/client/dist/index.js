...
// src/errors.ts
var BetterAuthClientError = class extends BetterAuthError {
  description;
...
// src/index.ts
function formatCbURL(callbackURL) {
  if (!callbackURL) {
    return window.location.origin;
  }
  return callbackURL.startsWith("http") ? callbackURL : `${window.location.origin}${callbackURL.startsWith("/") ? "" : "/"}${callbackURL}`;
}
...
var getAuthClient = (options) => {
  ...
  async function getCSRFToken() {
    const { data, error } = await $fetch("/csrf");
    if (error?.status === 404) {
      throw new BetterAuthClientError(
    ...
    if (error) {
      throw new BetterAuthClientError(error.message || "Failed to get CSRF token.");
    }
    return data.csrfToken;
  }
  async function signIn(input) {
    const csrfToken = await getCSRFToken();
    const { data, error } = await $fetch("/signin", {
      body: {
        ...input,
        csrfToken,
  ...
  async function signInOrSignUp(input) {
    const csrfToken = await getCSRFToken();
    const { data, error } = await $fetch("/signin", {
      body: {
        ...input,
  ...
  async function signUp(input) {
    const csrfToken = await getCSRFToken();
    const { data, error } = await $fetch("/signup", {
      body: {
        ...input,
        csrfToken,
        currentURL: window.location.href,
        callbackURL: formatCbURL(input?.callbackURL),
        autoCreateSession: input.autoCreateSession !== void 0 ? input.autoCreateSession : true
      }
    });
    if (error) {
      appendError(error.error);
  ...
  const signOut = async (input) => {
    const csrfToken = await getCSRFToken();
    const { data, error } = await $fetch("/signout", {
      body: {
        csrfToken
      }
    });
  ...
  const getSession = async () => {
    const csrfToken = await getCSRFToken();
    const { data, error } = await $fetch("/session", {
      method: "POST",
      body: {
        csrfToken
      }
